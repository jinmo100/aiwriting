...
CREATE TABLE IF NOT EXISTS background_jobs (
    ...
    CONSTRAINT ck_background_jobs_status CHECK (status IN ('PENDING', 'RUNNING', 'COMPLETED', 'FAILED', 'SKIPPED')),
    CONSTRAINT ck_background_jobs_attempt_count_non_negative CHECK (attempt_count >= 0),
    CONSTRAINT ck_background_jobs_max_attempts_non_negative CHECK (max_attempts >= 0)
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_background_jobs_active_business
...
CREATE INDEX IF NOT EXISTS idx_embedding_configs_owner_created_at
    ON embedding_configs(owner_user_id, created_at DESC);

CREATE UNIQUE INDEX IF NOT EXISTS ux_embedding_configs_owner_default
    ON embedding_configs(owner_user_id)
    WHERE is_default = true;

CREATE UNIQUE INDEX IF NOT EXISTS ux_embedding_configs_owner_id
    ON embedding_configs(owner_user_id, id);
...
CREATE INDEX IF NOT EXISTS idx_rag_documents_active_type
    ON rag_documents(is_active, document_type, essay_type);

CREATE INDEX IF NOT EXISTS idx_rag_documents_skill_tag
    ON rag_documents(skill_tag);

CREATE UNIQUE INDEX IF NOT EXISTS ux_rag_documents_skill_version
    ON rag_documents(skill_tag, version);
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_rag_chunks_document_chunk_no
...
CREATE TABLE IF NOT EXISTS rag_chunk_embeddings (
    ...
    CONSTRAINT fk_rag_chunk_embeddings_user_embedding_config
        FOREIGN KEY (user_id, embedding_config_id)
        REFERENCES embedding_configs(owner_user_id, id)
        ON DELETE CASCADE
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_rag_embeddings_user_config_chunk_version
    ON rag_chunk_embeddings(user_id, embedding_config_id, chunk_id, embedding_version);

CREATE INDEX IF NOT EXISTS idx_rag_embeddings_user_config
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_essays_user_id
    ON essays(user_id, id);

CREATE UNIQUE INDEX IF NOT EXISTS ux_essay_scores_essay_id
    ON essay_scores(essay_id, id);
...
CREATE TABLE IF NOT EXISTS rag_feedbacks (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_rag_feedbacks_user_essay
        FOREIGN KEY (user_id, essay_id)
        REFERENCES essays(user_id, id)
        ON DELETE CASCADE,
    CONSTRAINT fk_rag_feedbacks_essay_score
        FOREIGN KEY (essay_id, score_id)
        REFERENCES essay_scores(essay_id, id)
        ON DELETE CASCADE,
    CONSTRAINT fk_rag_feedbacks_user_embedding_config
        FOREIGN KEY (user_id, embedding_config_id)
        REFERENCES embedding_configs(owner_user_id, id)
        ON DELETE SET NULL (embedding_config_id)
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_rag_feedback_score_config
    ON rag_feedbacks(score_id, embedding_config_id)
    WHERE embedding_config_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_rag_feedbacks_user_essay_created_at
    ON rag_feedbacks(user_id, essay_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_rag_feedbacks_job_id
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_rag_feedback_citations_feedback_rank
    ON rag_feedback_citations(feedback_id, rank_no);

CREATE INDEX IF NOT EXISTS idx_rag_feedback_citations_chunk
...
COMMENT ON COLUMN rag_feedbacks.essay_id IS '反馈关联的作文 ID，数据库通过 (user_id, essay_id) 兜底校验作文归属';
COMMENT ON COLUMN rag_feedbacks.score_id IS '反馈关联的评分结果 ID，数据库通过 (essay_id, score_id) 兜底校验评分属于该作文';
COMMENT ON COLUMN rag_feedbacks.api_config_id IS '生成反馈复用的 Chat Provider 配置 ID；公开/共享配置可能跨用户可用，归属和可用性由 service 层结合 visibility/allow_public_use 校验';
COMMENT ON COLUMN rag_feedbacks.embedding_config_id IS '检索知识时使用的 Embedding 配置 ID，数据库通过 (user_id, embedding_config_id) 兜底校验配置归属，配置删除后置空';
...
COMMENT ON CONSTRAINT ck_background_jobs_status ON background_jobs IS '限制后台任务状态只能使用已登记枚举，避免拼写错误绕过活跃任务去重索引';
COMMENT ON CONSTRAINT ck_background_jobs_attempt_count_non_negative ON background_jobs IS '限制后台任务已尝试次数不能为负数';
COMMENT ON CONSTRAINT ck_background_jobs_max_attempts_non_negative ON background_jobs IS '限制后台任务最大尝试次数不能为负数';
COMMENT ON CONSTRAINT ck_embedding_configs_dimensions_1536 ON embedding_configs IS 'V1 仅支持 1536 维 Embedding，避免混写不同维度向量';
COMMENT ON CONSTRAINT ck_rag_chunk_embeddings_dimension_1536 ON rag_chunk_embeddings IS '向量表 V1 固定保存 1536 维向量';
COMMENT ON CONSTRAINT fk_rag_chunk_embeddings_user_embedding_config ON rag_chunk_embeddings IS '兜底保证知识片段向量只能使用同一用户拥有的 Embedding 配置';
COMMENT ON CONSTRAINT fk_rag_feedbacks_user_essay ON rag_feedbacks IS '兜底保证 RAG Feedback 只能关联同一用户拥有的作文';
COMMENT ON CONSTRAINT fk_rag_feedbacks_essay_score ON rag_feedbacks IS '兜底保证 RAG Feedback 的评分结果必须属于同一篇作文';
COMMENT ON CONSTRAINT fk_rag_feedbacks_user_embedding_config ON rag_feedbacks IS '兜底保证 RAG Feedback 使用的 Embedding 配置属于同一用户，避免跨用户检索配置串联';
...
COMMENT ON INDEX ux_embedding_configs_owner_id IS '为 RAG 向量和反馈表的用户归属复合外键提供唯一参照';
...
COMMENT ON INDEX ux_rag_documents_skill_version IS '同一知识库版本内每个稳定技能标签只保留一张内置知识卡，支持 seed upsert';
...
COMMENT ON INDEX ux_essays_user_id IS '为 RAG Feedback 校验作文用户归属提供唯一参照';
COMMENT ON INDEX ux_essay_scores_essay_id IS '为 RAG Feedback 校验评分属于指定作文提供唯一参照';
...
WITH seed_cards(document_type, title, source_type, source_title, essay_type, skill_tag, level_tag, version, content, content_hash, metadata_json) AS (
    ...
    SELECT
        ...
        TRUE
    FROM seed_cards
    ON CONFLICT (skill_tag, version) DO UPDATE SET
...
SELECT
    ...
    TRUE
FROM seed_cards
JOIN upserted_documents
    ON upserted_documents.skill_tag = seed_cards.skill_tag
    AND upserted_documents.version = seed_cards.version
