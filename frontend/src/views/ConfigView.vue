      ...
      <el-table :data="configStore.configs" v-loading="configStore.loading">
        ...
        <el-table-column label="Provider" min-width="170">
          <template #default="{ row }">
            ...
            <small v-if="row.providerLabel" class="muted">{{
              row.providerLabel
            }}</small>
          </template>
        </el-table-column>
        ...
        <el-table-column label="测试状态" min-width="160">
          <template #default="{ row }">
            ...
            <div v-if="row.lastTestMessage" class="test-message">
              {{ row.lastTestMessage }}
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="360">
          <template #default="{ row }">
            ...
            <el-button size="small" type="primary" @click="handleTestSaved(row)"
              >测试</el-button
            >
            <el-button
              size="small"
              type="info"
              @click="handleFetchModelsSaved(row)"
              >模型</el-button
            >
            <el-button
              size="small"
    ...
    >
      <el-form :model="form" label-width="140px">
        <el-form-item label="配置名称" required>
          <el-input v-model="form.configName" placeholder="例如：openai官方" />
        </el-form-item>
        ...
        <el-form-item label="提供商名称">
          <el-input
            ...
            placeholder="例如：OpenRouter、openai官方、Anthropic"
          />
        </el-form-item>
        ...
        <el-form-item label="API Base URL" required>
          <el-input
            ...
            placeholder="请填写 API 根地址，不要包含具体 endpoint"
          />
          <div class="hint">{{ baseUrlHint }}</div>
        </el-form-item>

        <el-form-item
          :label="editId ? 'API Key' : 'API Key'"
          :required="!editId"
        >
          <el-input
            v-model="form.apiKey"
            ...
            show-password
            :placeholder="
              editId ? '留空表示不修改，填写新 Key 表示替换' : '输入 API Key'
            "
          />
          <div v-if="editingConfig?.apiKeyPreview" class="hint">
            已保存：{{ editingConfig.apiKeyPreview }}
          </div>
          <el-button
        ...
        <el-row :gutter="12">
          <el-col :span="8">
            <el-form-item label="输入单价/百万">
              <el-input-number
                v-model="form.inputTokenPricePerMillion"
                :min="0"
                :precision="6"
                :step="0.1"
                class="full-width"
          ...
          <el-col :span="8">
            <el-form-item label="输出单价/百万">
              <el-input-number
                v-model="form.outputTokenPricePerMillion"
                :min="0"
                :precision="6"
                :step="0.1"
                class="full-width"
          ...
          <el-col :span="8">
            <el-form-item label="币种">
              <el-input v-model="form.currency" placeholder="例如 USD / CNY" />
            </el-form-item>
          </el-col>
        </el-row>
        <div class="cost-hint">
          费用仅按你自己填写的单价估算；未填写单价时结果页只展示 Token 消耗。
        </div>
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button @click="handleTestConnection" :loading="testingConnection"
            >测试连接</el-button
          >
          <el-button @click="handleTestStructured" :loading="testingStructured"
            >测试输出格式</el-button
          >
...
const providerOptions: Array<{
...
}> = [
  {
    ...
    endpointHint:
      "请输入 API 根地址，例如 https://api.openai.com/v1，不要包含 /chat/completions。",
  },
  {
    ...
    endpointHint:
      "请输入 API 根地址，例如 https://api.openai.com/v1，不要包含 /responses。",
  },
  {
    ...
    endpointHint:
      "请输入 API 根地址，例如 https://api.anthropic.com/v1，不要包含 /messages。",
  },
  {
    label: "Google Gemini",
    ...
    endpointHint:
      "请输入 API 根地址，例如 https://generativelanguage.googleapis.com/v1beta，不要包含 /models/{model}:generateContent。",
  },
];
...
const currentProviderOption = computed(
  () =>
    providerOptions.find((item) => item.value === form.value.providerType) ||
    providerOptions[0],
);
...
async function handleFetchModels(forceRefresh: boolean) {
  ...
  try {
    const result =
      editId.value && !form.value.apiKey
...
async function handleTestConnection() {
  ...
  try {
    const request =
      editId.value && !form.value.apiKey ? null : buildTestRequest();
...
async function handleSubmit() {
  ...
  try {
    const payload: ApiConfigRequest = {
      ...form.value,
      currency: form.value.currency?.trim() || undefined,
      inputTokenPricePerMillion: form.value.inputTokenPricePerMillion ?? undefined,
      outputTokenPricePerMillion: form.value.outputTokenPricePerMillion ?? undefined,
    };
